...
func (t *TheTVDBApiImpl) GetSeriesTranslation(ctx context.Context, seriesID string, lang string) (*Translation, error) {
	var responseData Response[Translation]
	// create http request with authorization token
	req, err := http.NewRequest("GET", t.baseURL+"/series/"+seriesID+"/translations/"+lang, nil)
	if err != nil {
		return nil, err
	}
